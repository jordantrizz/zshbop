...
function _proxmox_createvm () {
    ...
    if [[ $NETWORK_CI == "auto" ]]; then
        _loading2 "Network cloud-init is set to $NETWORK_CI"
        NETWORK_CI_CLI=""
    elif [[ $NETWORK_CI == "private" ]]; then
        _loading2 "Network cloud-init is set to $NETWORK_CI adding --cicustom network=snippets/network-$VM_ID.yaml to build"        
        NETWORK_CI_CLI="network=local:snippets/network-$VM_ID.yaml"
    else
    ...
    if [[ $NETWORK_CI == "auto" ]]; then
        ...
        VM_MAC_UPPER=$(qm config $VM_ID | grep 'net0' | awk '{ print $2 }' | awk -F',' '{ print $1 }' | awk -F'=' '{ print $2 }')
        # Convert VM_MAC to lower case
        VM_MAC="${VM_MAC_UPPER:l}"
        _loading3 "MAC address is $VM_MAC_UPPER changing to lowercase $VM_MAC"
